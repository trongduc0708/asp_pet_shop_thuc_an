...
class ItemDataset(Dataset):
    def __init__(self, csv_path):
        df = pd.read_csv(csv_path)
        self.df = df
        self.item2idx = {it: i for i, it in enumerate(df["item_id"].unique())}
        self.idx2item = {v: k for k, v in self.item2idx.items()}
...
class HybridRecommender:
    def __init__(self, cf_path, cbf_path, cf_data, cbf_data, alpha=0.6):
        ...
        self.item2idx_cbf = cbf_ds.item2idx
        self.idx2item_cbf = cbf_ds.idx2item
        self.alpha = alpha
...
def load_models():
    ...
    try:
        ...
        cf_path = os.path.join(base_path, "cvae_cf_model.pt")
        cbf_path = os.path.join(base_path, "cvae_cbf_model.pt")
        cf_data = os.path.join(base_path, "interactions_cvae_cf_large.csv")
        cbf_data = os.path.join(base_path, "items_cvae_cbf.csv")
...
@app.get("/")
async def root():
    return {"message": "Pet Shop Recommendation API", "status": "running"}
...
async def get_recommendations(request: RecommendationRequest):
    ...
    try:
        # Convert user_id và item_id nếu cần
        user_id = request.user_id
        
        # Get recommendations
        recommendations = recommender.hybrid_recommend(
            user_id=user_id,
            current_item=request.current_item_id,
            k=request.k
        )
...
async def get_batch_recommendations(requests: List[RecommendationRequest]):
    ...
    for req in requests:
        try:
            recommendations = recommender.hybrid_recommend(
                user_id=req.user_id,
                current_item=req.current_item_id,
                k=req.k
            )
            results.append({
                "user_id": req.user_id,
