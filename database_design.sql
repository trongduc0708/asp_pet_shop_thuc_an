...
-- =====================================================
-- 12. TẠO STORED PROCEDURES CƠ BẢN
-- =====================================================
-- Lưu ý: Các báo cáo doanh thu và thống kê sẽ được tính toán động từ dữ liệu Orders và OrderItems

-- Stored procedure cập nhật tồn kho
GO
...
BEGIN
    ...
    DECLARE @ShippingFee DECIMAL(15,2);
    ...
    -- Tính toán giá trị đơn hàng từ giỏ hàng
    SELECT @SubTotal = ISNULL(SUM(c.Quantity * p.Price), 0)
    FROM Cart c
    INNER JOIN Products p ON c.ProductID = p.ProductID
    WHERE c.UserID = @UserID;
    
    -- Tính phí ship (ví dụ: miễn phí ship từ 500k)
    ...
    BEGIN
        -- Logic tính giảm giá dựa trên loại khuyến mãi
        -- (Có thể mở rộng thêm)
    END;
    ...
    -- Tạo đơn hàng
    INSERT INTO Orders (OrderNumber, UserID, StatusID, PaymentMethodID, SubTotal, ShippingFee, DiscountAmount, TotalAmount, PromotionID, ShippingAddress)
    VALUES (@OrderNumber, @UserID, 1, @PaymentMethodID, @SubTotal, @ShippingFee, @DiscountAmount, @TotalAmount, @PromotionID, @ShippingAddress);
    ...
    -- Cập nhật tồn kho
    UPDATE Inventory 
    SET QuantityInStock = QuantityInStock - c.Quantity,
        LastUpdated = GETDATE()
    FROM Cart c
    WHERE Inventory.ProductID = c.ProductID AND c.UserID = @UserID;
END
GO

-- =====================================================
-- KẾT THÚC THIẾT KẾ CƠ SỞ DỮ LIỆU
-- =====================================================
-- Lưu ý: 
-- - FAQ đã được loại bỏ, sẽ fix cứng trong code
-- - Báo cáo doanh thu và thống kê sẽ được tính toán động từ dữ liệu Orders và OrderItems
-- - Database đã được tối ưu để giảm thiểu các bảng không cần thiết
